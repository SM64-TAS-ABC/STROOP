            ...
            foreach (IDataContainer specialVar in _specialWatchVars)
            {
                switch (specialVar.SpecialName)
                {
                    ...
                    case "DistanceToLine12":
                        {
                            double signedDistToLine = MoreMath.GetSignedDistanceFromPointToLine(marioX, marioZ, v1X, v1Z, v2X, v2Z, v3X, v3Z, 1, 2, true);
                            (specialVar as DataContainer).Text = Math.Round(signedDistToLine, 3).ToString();
                        }
                        goto case "CheckTriangleExists";
                    case "DistanceToLine23":
                        {
                            double signedDistToLine = MoreMath.GetSignedDistanceFromPointToLine(marioX, marioZ, v1X, v1Z, v2X, v2Z, v3X, v3Z, 2, 3, true);
                            (specialVar as DataContainer).Text = Math.Round(signedDistToLine, 3).ToString();
                        }
                        goto case "CheckTriangleExists";
                    case "DistanceToLine13":
                        {
                            double signedDistToLine = MoreMath.GetSignedDistanceFromPointToLine(marioX, marioZ, v1X, v1Z, v2X, v2Z, v3X, v3Z, 3, 1, true);
                            (specialVar as DataContainer).Text = Math.Round(signedDistToLine, 3).ToString();
                        }
                        goto case "CheckTriangleExists";
