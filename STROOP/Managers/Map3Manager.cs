    ...
    public class Map3Manager
    {
        ...
        public void UpdateFromMarioTab()
        {
            ...
            for (int numSides = SHAPE_MIN_SIDES; numSides <= SHAPE_MAX_SIDSE; numSides++)
            {
                (List<TriangleShape> floors, List<TriangleShape> walls) = GetTriShapes(numSides);
                int index = numSides - SHAPE_MIN_SIDES;
                List<TriangleMap3Object> floorTris = _triObjectFloors[index];
                List<TriangleMap3Object> wallTris = _triObjectWalls[index];
        ...
        public void Update(bool updateView)
        {
            ...
            foreach (TriangleMap3Object cogFloorTri in _cogFloorTris)
            {
            ...
            foreach (TriangleMap3Object cogFloorTri in _cog2FloorTris)
            {
                cogFloorTri.P1OnControl = CalculateLocationOnControl(new PointF(cogFloorTri.RelX1, cogFloorTri.RelZ1), mapView);
                cogFloorTri.P2OnControl = CalculateLocationOnControl(new PointF(cogFloorTri.RelX2, cogFloorTri.RelZ2), mapView);
                cogFloorTri.P3OnControl = CalculateLocationOnControl(new PointF(cogFloorTri.RelX3, cogFloorTri.RelZ3), mapView);
            ...
            foreach (TriangleMap3Object cogWallTri in _cogWallTris)
            {
            ...
            foreach (List<TriangleMap3Object> tris in _triObjectFloors)
            {
                foreach (TriangleMap3Object tri in tris)
                {
            ...
            foreach (List<TriangleMap3Object> tris in _triObjectWalls)
            {
                foreach (TriangleMap3Object tri in tris)
                {
                    tri.P1OnControl = CalculateLocationOnControl(new PointF(tri.RelX1, tri.RelZ1), mapView);
            ...
            foreach (ObjectDataModel obj in DataModels.Objects)
            {
                ...
                /*
                // Need to add map obj
                if (Config.ObjectSlotsManager.ShowHomeOnMap3SlotsAddresses.Contains(address) &&
                    !_mapObjectHomeDictionary.ContainsKey(address))
                {
                    Map3Object mapObj = new Map3Object(Config.ObjectAssociations.HomeImage);
                ...
                if (!Config.ObjectSlotsManager.ShowHomeOnMap3SlotsAddresses.Contains(address) &&
                    _mapObjectHomeDictionary.ContainsKey(address))
                {
                    Map3Object mapObj = _mapObjectHomeDictionary[address];
